...
class BaseSamCliLocator {
    ...
    findFileInFolders(files, folders) {
        return __awaiter(this, void 0, void 0, function* () {
            const fullPaths = files.map(file => folders
                .filter(folder => !!folder)
                .map(folder => path.join(folder, file))).reduce((accumulator, paths) => {
    ...
    getSystemPathLocation() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (!!envVars.PATH) {
                const systemPaths = envVars.PATH.split(path.delimiter)
                    .filter(folder => !!folder);
...
WindowsSamCliLocator.EXECUTABLE_FILENAMES = [
    'sam.cmd',
    'sam.exe'
];
...
UnixSamCliLocator.LOCATION_PATHS = [
    '/usr/local/bin',
    '/usr/bin'
];
UnixSamCliLocator.EXECUTABLE_FILENAMES = [
    'sam'
];
