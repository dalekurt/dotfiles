...
function initialize({ configuration, outputChannel: toolkitOutputChannel, processInvoker = new samCliInvoker_1.DefaultSamCliProcessInvoker(), telemetryService, localInvokeCommand = new samCliLocalInvoke_1.DefaultSamLocalInvokeCommand(vsCodeUtils_1.getChannelLogger(toolkitOutputChannel), [samCliLocalInvoke_1.WAIT_FOR_DEBUGGER_MESSAGES.DOTNET]), }) {
...
function getCodeUri(resource, samTemplateUri) {
    ...
    return path.isAbsolute(rawCodeUri) ?
        rawCodeUri :
        path.join(path.dirname(samTemplateUri.fsPath), rawCodeUri);
}
/**
