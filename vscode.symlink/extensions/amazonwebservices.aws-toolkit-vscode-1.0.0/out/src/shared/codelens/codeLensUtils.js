...
function makeCodeLenses({ document, token, handlers, language }) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        for (const handler of handlers) {
            ...
            const range = (handler.range instanceof vscode.Range) ? handler.range : new vscode.Range(document.positionAt(handler.range.positionStart), document.positionAt(handler.range.positionEnd));
            try {
...
function getMetricDatum({ isDebug, runtime }) {
    return {
        datum: Object.assign({}, telemetryUtils_1.defaultMetricDatum('invokelocal'), { metadata: new Map([
                ['runtime', runtime],
                ['debug', `${isDebug}`]
            ]) })
    };
